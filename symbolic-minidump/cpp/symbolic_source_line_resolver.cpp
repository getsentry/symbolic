...
extern "C" {
void *cfi_rules_new();
void cfi_rules_free(void *cfi_rules);
bool cfi_rules_insert(void *cfi_rules, const char *record_string);
}
...
bool TokenizeWithOptionalField(char *line,
                               ...
                               int max_tokens,
                               vector<char *> *tokens) {
    // First tokenize assuming the optional field is not present.  If we then
    // see the optional field, additionally tokenize the last token into two
    // tokens.
    ...
    if (strcmp(tokens->front(), optional_field) == 0) {
        // The optional field is present. Split the last token in two to recover
        // the field prior to the last.
...
SymbolicSourceLineResolver::SymbolicSourceLineResolver(bool is_big_endian)
    : SourceLineResolverBase(new SymbolicModuleFactory(is_big_endian)) {
}

SymbolicSourceLineResolver::Module::Module(const string &name,
                                           bool is_big_endian)
    : name_(name), is_corrupt_(false) {
    is_big_endian_ = is_big_endian;
    cfi_rules_ = cfi_rules_new();
}

SymbolicSourceLineResolver::Module::~Module() {
    cfi_rules_free(cfi_rules_);
}

// static
void SymbolicSourceLineResolver::Module::LogParseError(const string &message,
                                                       int line_number,
                                                       int *num_errors) {
...
bool SymbolicSourceLineResolver::Module::LoadMapFromMemory(
    char *memory_buffer, size_t memory_buffer_size) {
    ...
    while (buffer != NULL) {
        ...
        if (strncmp(buffer, "FILE ", 5) == 0) {
            if (!ParseFile(buffer)) {
                LogParseError("ParseFile on buffer failed", line_number,
                              &num_errors);
            }
        } else if (strncmp(buffer, "STACK ", 6) == 0) {
            if (!ParseStackInfo(buffer)) {
                LogParseError("ParseStackInfo failed", line_number,
                              &num_errors);
            }
            ...
            if (!cur_func.get()) {
                ...
                // StoreRange will fail if the function has an invalid address
                // or size. We'll silently ignore this, the function and any
                // corresponding lines will be destroyed when cur_func is
                // released.
                functions_.StoreRange(cur_func->address, cur_func->size,
                                      cur_func);
            }
        } else if (strncmp(buffer, "PUBLIC ", 7) == 0) {
            // Clear cur_func: public symbols don't contain line number
            // information.
            cur_func.reset();

            if (!ParsePublicSymbol(buffer)) {
                LogParseError("ParsePublicSymbol failed", line_number,
                              &num_errors);
            }
        } else if (strncmp(buffer, "MODULE ", 7) == 0) {
            // Ignore these.  They're not of any use to
            // SymbolicSourceLineResolver, which is fed modules by a
            // SymbolSupplier.  These lines are present to aid other tools in
            // properly placing symbol files so that they can be accessed by a
            // SymbolSupplier.
            //
...
void SymbolicSourceLineResolver::Module::LookupAddress(
        ...
        if (func->lines.RetrieveRange(address, &line, &line_base,
            ...
            if (it != files_.end()) {
                frame->source_file_name =
                    files_.find(line->source_file_id)->second;
            }
            frame->source_line = line->line;
            frame->source_line_base = frame->module->base_address() + line_base;
        }
    } else if (public_symbols_.Retrieve(address, &public_symbol,
                                        &public_address) &&
...
WindowsFrameInfo *SymbolicSourceLineResolver::Module::FindWindowsFrameInfo(
    ...
    if ((windows_frame_info_[WindowsFrameInfo::STACK_INFO_FRAME_DATA]
        ...
        (windows_frame_info_[WindowsFrameInfo::STACK_INFO_FPO].RetrieveRange(
            address, &frame_info))) {
...
CFIFrameInfo *SymbolicSourceLineResolver::Module::FindCFIFrameInfo(
    const StackFrame *frame) const {
    MemAddr address = frame->instruction - frame->module->base_address();
    return new SymbolicCFIFrameInfo(is_big_endian_, cfi_rules_, address);
}

bool SymbolicSourceLineResolver::Module::ParseFile(char *file_line) {
    long index;
    char *filename;
...
SymbolicSourceLineResolver::Function *
SymbolicSourceLineResolver::Module::ParseFunction(char *function_line) {
    bool is_multiple;
    uint64_t address;
    uint64_t size;
...
SymbolicSourceLineResolver::Line *SymbolicSourceLineResolver::Module::ParseLine(
    char *line_line) {
    uint64_t address;
    uint64_t size;
    long line_number;
...
bool SymbolicSourceLineResolver::Module::ParsePublicSymbol(char *public_line) {
    bool is_multiple;
    uint64_t address;
    long stack_param_size;
    char *name;

    if (SymbolParseHelper::ParsePublicSymbol(
            public_line, &is_multiple, &address, &stack_param_size, &name)) {
        // A few public symbols show up with an address of 0.  This has been
        // seen in the dumped output of ntdll.pdb for symbols such as _CIlog,
        // _CIpow, RtlDescribeChunkLZNT1, and RtlReserveChunkLZNT1.  They would
        // conflict with one another if they were allowed into the
        // public_symbols_ map, but since the address is obviously invalid,
        // gracefully accept them as input without putting them into the map.
        if (address == 0) {
            return true;
        }

        linked_ptr<PublicSymbol> symbol(
            new PublicSymbol(name, address, stack_param_size, is_multiple));
        return public_symbols_.Store(address, symbol);
    }
    return false;
}

bool SymbolicSourceLineResolver::Module::ParseStackInfo(char *stack_info_line) {
    // Try parsing a CFI record FIRST. The code after this will modify the input
    // string.
    if (cfi_rules_insert(cfi_rules_, stack_info_line)) {
        return true;
    }
    ...
    // information this is.
    while (*stack_info_line == ' ') stack_info_line++;
    const char *platform = stack_info_line;
    while (!strchr(kWhitespace, *stack_info_line)) stack_info_line++;
    *stack_info_line++ = '\0';
    ...
    if (strcmp(platform, "WIN") == 0) {
        ...
        linked_ptr<WindowsFrameInfo> stack_frame_info(
            WindowsFrameInfo::ParseFromString(stack_info_line, type, rva,
                                              code_size));
        if (stack_frame_info == NULL) return false;
        ...
        // in strncpy_s in test_app.cc (testdata/minidump2).  There, problem
        // looks like this:
        //   STACK WIN 4 4242 1a a 0 ...  (STACK WIN 4 base size prolog 0 ...)
        //   STACK WIN 4 4243 2e 9 0 ...
        // ContainedRangeMap treats these two blocks as conflicting.  In
        // reality, when the prolog lengths are taken into account, the actual
        // code of these blocks doesn't conflict.  However, we can't take the
        // prolog lengths into account directly here because we'd wind up with a
        // different set of range conflicts when MSVC outputs stack info like
        // this:
        //   STACK WIN 4 1040 73 33 0 ...
        //   STACK WIN 4 105a 59 19 0 ...
        // because in both of these entries, the beginning of the code after the
        // prolog is at 0x1073, and the last byte of contained code is at
        // 0x10b2. Perhaps we could get away with storing ranges by rva +
        // prolog_size if ContainedRangeMap were modified to allow replacement
        // of already-stored values.
...
bool SymbolicSourceLineResolver::Module::ParseCFIFrameInfo(
    char *stack_info_line) {
    return false;
}

// static
bool SymbolParseHelper::ParseFile(char *file_line,
                                  long *index,
                                  char **filename) {
    // FILE <id> <filename>
...
// static
bool SymbolParseHelper::ParseFunction(char *function_line,
                                      bool *is_multiple,
                                      uint64_t *address,
                                      uint64_t *size,
                                      long *stack_param_size,
    ...
    vector<char *> tokens;
    if (!TokenizeWithOptionalField(function_line, "m", kWhitespace, 5,
                                   &tokens)) {
        return false;
    }
...
// static
bool SymbolParseHelper::ParseLine(char *line_line,
                                  uint64_t *address,
                                  uint64_t *size,
                                  long *line_number,
                                  long *source_file) {
    ...
    *source_file = strtol(tokens[3], &after_number, 10);
    ...
    // Valid line numbers normally start from 1, however there are functions
    // that are associated with a source file but not associated with any line
    // number (block helper function) and for such functions the symbol file
    // contains 0 for the line numbers.  Hence, 0 should be treated as a valid
    // line number. For more information on block helper functions, please, take
    // a look at: http://clang.llvm.org/docs/Block-ABI-Apple.html
    if (*line_number < 0) {
        return false;
    }
