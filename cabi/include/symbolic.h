...
enum SymbolicErrorCode {
    ...
    SYMBOLIC_ERROR_CODE_PROCESS_MINIDUMP_ERROR_DUPLICATE_REQUESTING_THREADS =
        4006,
    SYMBOLIC_ERROR_CODE_PROCESS_MINIDUMP_ERROR_SYMBOL_SUPPLIER_INTERRUPTED =
        4007,
    ...
    SYMBOLIC_ERROR_CODE_APPLE_CRASH_REPORT_PARSE_ERROR_INVALID_INCIDENT_IDENTIFIER =
        8002,
    SYMBOLIC_ERROR_CODE_APPLE_CRASH_REPORT_PARSE_ERROR_INVALID_REPORT_VERSION =
        8003,
...
/*
 * Returns the kind of debug data contained in this object file, if any (e.g.
 * DWARF).
 */
...
/*
 * Returns the desiganted use of the object file and hints at its contents (e.g.
 * debug, executable, ...).
 */
...
const SymbolicUnreal4CrashFile *symbolic_unreal4_crash_file_by_index(
    const SymbolicUnreal4Crash *unreal, uintptr_t idx);

uintptr_t symbolic_unreal4_crash_file_count(const SymbolicUnreal4Crash *unreal);

const uint8_t *symbolic_unreal4_crash_file_meta_contents(
    const SymbolicUnreal4CrashFile *meta,
    const SymbolicUnreal4Crash *unreal,
    uintptr_t *len);

SymbolicStr symbolic_unreal4_crash_file_meta_name(
    const SymbolicUnreal4CrashFile *meta);

SymbolicStr symbolic_unreal4_crash_file_meta_type(
    const SymbolicUnreal4CrashFile *meta);
...
SymbolicStr symbolic_unreal4_crash_get_apple_crash_report(
    const SymbolicUnreal4Crash *unreal);

SymbolicProcessState *symbolic_unreal4_crash_process_minidump(
    const SymbolicUnreal4Crash *unreal);
