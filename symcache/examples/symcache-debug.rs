...
fn execute(matches: &ArgMatches) -> Result<(), Box<Error>> {
    ...
    if let Some(file_path) = matches.value_of("debug_file_path") {
        ...
        if arch == Arch::Unknown && objects.len() != 1 {
            ...
            for obj in objects {
                println!("  {} [{}]", obj.uuid().unwrap_or(Default::default()), obj.arch());
            }
            return Ok(());
        }
        ...
        if matches.is_present("write_cache_file") {
            let filename = matches.value_of("symcache_file_path")
                .map(|x| x.to_string())
...
fn main() {
    let matches = App::new("symcache-debug")
        ...
        .arg(Arg::with_name("debug_file_path")
             .short("d")
             .long("debug-file")
             .value_name("PATH")
             .help("Path to the debug info file"))
        .arg(Arg::with_name("write_cache_file")
             .short("w")
             .long("write-cache-file")
             .help("Write the cache file from the debug info file.  If no file name is \
        ...
        .arg(Arg::with_name("symcache_file_path")
             .short("c")
             .long("symcache-file")
             .value_name("PATH")
             .help("Path to the symcache file"))
        .arg(Arg::with_name("lookup_addr")
             .long("lookup")
             .value_name("ADDR")
             .help("Looks up an address in the debug file"))
        .arg(Arg::with_name("print_symbols")
             .long("symbols")
