...
use symbolic_smcache::{
    extract_scope_names, NameResolver, ScopeIndex, ScopeLookupResult, SourceContext, SourcePosition,
};
use symbolic_smcache::{SmCache, SmCacheWriter};
use symbolic_testutils::fixture;

#[test]
fn resolves_scope_names() {
    let src = std::fs::read_to_string(fixture("smcache/trace/sync.mjs")).unwrap();
...
fn resolves_token_from_names() {
    let minified = std::fs::read_to_string(fixture("smcache/preact.module.js")).unwrap();
    let ctx = SourceContext::new(&minified).unwrap();

    let map = std::fs::read_to_string(fixture("smcache/preact.module.js.map")).unwrap();
...
fn writes_simple_cache() {
    let minified = std::fs::read_to_string(fixture("smcache/simple/minified.js")).unwrap();
    let map = std::fs::read_to_string(fixture("smcache/simple/minified.js.map")).unwrap();
...
fn missing_source_contents() {
    ...
    let map = std::fs::read_to_string(fixture(
        "smcache/preact-missing-source-contents.module.js.map",
    ))
    .unwrap();
...
fn hermes_scope_lookup() {
    ...
    let map =
        std::fs::read_to_string(fixture("smcache/hermes-metro/react-native-hermes.map")).unwrap();
...
fn metro_scope_lookup() {
    ...
    let minified =
        std::fs::read_to_string(fixture("smcache/hermes-metro/react-native-metro.js")).unwrap();
    let map =
        std::fs::read_to_string(fixture("smcache/hermes-metro/react-native-metro.js.map")).unwrap();
