...
fn test_perforce_transformation() {
    ...
    use symbolic_symcache::transform::{File, SourceLocation, Transformer};
...
"#;
    ...
    let mut mapper = PerforcePathMapper::from_srcsrv_data(srcsrv_data)
        .expect("Failed to parse SRCSRV data");

    // Create a source location that matches the SRCSRV entry
    let file = File::new_for_test(
        Cow::Borrowed("main.cpp"),
        Some(Cow::Borrowed("src")),
        Some(Cow::Borrowed("C:/build/game")),
    );
    let source_loc = SourceLocation::new_for_test(file, 10);
...
"#;
    ...
    let file = File::new_for_test(
        Cow::Borrowed("unknown.cpp"),
        Some(Cow::Borrowed("other")),
        Some(Cow::Borrowed("D:/different/path")),
    );
    let source_loc = SourceLocation::new_for_test(file, 5);
...
fn test_perforce_e2e_with_pdb_and_symcache() -> Result<(), Error> {
    ...
    // The fixture should be created on Windows using:
    // 1. Build crash.pdb with debug symbols
    // 2. Use pdbstr.exe to inject SRCSRV data:
    //    pdbstr -w -p:crash_with_srcsrv.pdb -i:srcsrv.txt -s:srcsrv
    //
    // Example srcsrv.txt content:
    // SRCSRV: ini ------------------------------------------------
    // VERSION=1
    // VERCTRL=Perforce
    // SRCSRV: variables ------------------------------------------
    // SRCSRV: source files ---------------------------------------
    // C:\Users\Default\Projects\Sentry\crash.cpp*P4*depot/sentry/crash.cpp*12345
    // SRCSRV: end ------------------------------------------------

    let view = ByteView::open("../symbolic-debuginfo/tests/fixtures/windows/crash_with_srcsrv.pdb")?;
    let pdb = PdbObject::parse(&view)?;

    // Extract raw SRCSRV data from PDB
    let srcsrv_bytes = pdb.source_server_data()?.expect(
        "crash_with_srcsrv.pdb should have SRCSRV data. \
         Create it on Windows with pdbstr.exe (see CREATE_SRCSRV_PDB.md)"
    );

    let srcsrv_data = std::str::from_utf8(&srcsrv_bytes)?;

    // Create Perforce transformer
    let mapper = PerforcePathMapper::from_srcsrv_data(srcsrv_data).expect(
        "Should be able to parse SRCSRV data into PerforcePathMapper"
    );
    ...
    for addr in 0..0x10000u64 {
        if let Some(sl) = cache.lookup(addr).next() {
            if let Some(file) = sl.file() {
                ...
                // Check if path starts with // (Perforce depot path)
                if path.starts_with("//depot/") || path.starts_with("//") {
                    found_depot_path = true;
                    eprintln!("Found transformed depot path: {}", path);
                    break;
                }
            }
        }
    ...
    assert!(
        found_depot_path,
        "Expected to find at least one file path transformed to Perforce depot format (//depot/...)"
    );
