...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_access_a_non_object_as_an_object(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_execute_a_privileged_instruction(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_execute_an_undefined_instruction(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_corrupt_malloc_s_internal_tracking_information(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_corrupt_the_objective_c_runtime_s_structures(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
...
@pytest.mark.parametrize("version, build, arch", TEST_PARAMETER)
def test_overwrite_link_register_then_crash(res_path, make_report_sym, version, build, arch):
    bt, report = _load_dsyms_and_symbolize_stacktrace(
